...
const DepartmentAdd = React.memo(() => {
	...
	const onDepartmentSubmit = React.useCallback(e => {
		e.preventDefault();
		...
		dispatch(postItem({
			...
			/** Slice 의 postItem.fulfilled 가 먼저 실행된 후 이 곳이 실행된다. */
			// result.meta --> 백엔드에게 전송한 파라미터
			// result.payload --> 백엔드로부터 전송받은 응답결과
			console.log(result);

			// 처리가 완료된 후 상세 페이지로 이동
			// --> 몇 번 데이터인지 path 파라미터로 전달한다. (querystring 도 가능함)
			navigate(`${window.location.href}/department_view/${result.payload.id}`);
		})
	}, []);
